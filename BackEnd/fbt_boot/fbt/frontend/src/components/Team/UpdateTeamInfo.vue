<template>
  <div class="py-0 my-0 px-0 mx-5">
          ...
          <v-card class="mx-5 py-4" v-if="page == 1">
            ...
            <v-row>
              <v-file-input
                ...
                v-if="doChangeImg"
              ></v-file-input>
              <v-row v-else class="mx-15">
                <div id="emblemLabel" class="mb-1">
                  앰블럼
                </div>
                <v-menu
                  :close-on-click="true"
                ...
                >
                  ...
                  <v-list>
                    <v-list-item v-for="(option, j) in options" :key="j">
                      <span
                        class="auth-menu"
    ...
    <!-- 팀정보 끝 -->
    <!-- progress circle -->
    <progress-circle v-else class="my-10"></progress-circle>
    <!-- progress circle 끝 -->
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      // 이미지 관련 변수
      doChangeImg: false, // 이미지 바꾸겠다고 하면 v-file 태그 활성화
      isHover: null,
      options: ["이미지 바꾸기", "기본 이미지"],
      disable: false, // 기본 이미지일 경우 true
      beforeUrl: null, // 이미지를 변경할 경우 전의 url를 입력해서 보낸다
  ...
  computed: {
    ...
    imgPlaceHolder: function() {
      if (this.team) {
        if (this.team.emblem != "emptyFC.svg")
          return "앰블럼을 업로드해주세요.";
      }
      return "기본 이미지";
    }
  ...
  methods: {
    ...
    submit() {
      ...
      this.$axios
        .put("/team/1/" + this.beforeUrl, fd, {
          headers: {
            "Content-Type": "multipart/form-data"
          }
        })
        .then(() => {
          alert("팀정보 수정이 완료됐습니다.");
        })
        .catch(error => {
          console.log(error);
          alert("팀정보 수정에 실패했습니다.");
        })
        .finally(() => {
          location.reload();
        });
    ...
    // 이미지 바꾸기
    changeImg(j) {
      this.beforeUrl = this.team.emblem;
      if (j == 1) {
        this.emblemFile = null;
        this.team.emblem = "emptyFC.svg";
        this.disable = true;
      }
