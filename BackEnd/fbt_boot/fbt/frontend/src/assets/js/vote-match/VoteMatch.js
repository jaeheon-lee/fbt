...
export default {
  ...
  methods: {
      ...
      this.$axios
        ...
        .then(response => {
          ...
          for(let i=0; i<this.votes.length;i++) {
            this.setAwayEmblemTemp(this.votes[i], teamId);
          }
        })
        .catch(() => {
          this.errored = true;
    ...
    setAwayEmblemTemp(vote, teamId) {
      let homeTeamId = vote.matchSchedule.homeTeam.teamId;
      if (teamId != homeTeamId) {
        this.getTeamInfo(vote, teamId);
      }
    },
    getTeamInfo(vote, teamId) {
      let teamMemberId = JSON.parse(sessionStorage.getItem("userInfo")).teamMemberId;
      this.$axios
        .get("/team/3/" + teamId + "/" + teamMemberId)
        .then(response => {
          vote.matchSchedule.awayTeam = response.data;
        })
    }
  }
};
